...
//
// SQL keywords are case-insensitive, so we match both upper and lower case
// but return the uppercase form to match the keyword map.
//
...
const KEYWORDS = Object.keys(SQL_KEYWORD_EMOJI_MAP);

// Sort longest-first so that e.g. "INTEGER" matches before "INT".
...
// Group 3 – single-quoted string     '...' (with '' escape)
// Group 4 – double-quoted identifier "..." (with "" escape)
// Group 5 – keyword                  \bKEYWORD\b (case-insensitive)
...
const TOKEN_REGEX = new RegExp(
  [
    ...
    `\\b(${KEYWORD_ALT})\\b`,                   // 5  keyword (case-insensitive)
  ].join('|'),
 ...
 */
function scanSqlKeywords(document) {
  ...
  while ((match = TOKEN_REGEX.exec(text)) !== null) {
    ...
    // Normalize to uppercase to match the keyword map
